...
from model import Denoiser, make_noisy_batch, make_scheduler, sample
...
def loss_fn(pred, noise, mask):
    sq = (pred - noise).pow(2) * mask
    return sq.sum() / (mask.sum() * pred.shape[1])
...
def save_samples(path, model, scheduler, x, y, steps):
    masked = x[:, :3]
    mask = x[:, 3:4]
    generated = sample(model, x, scheduler, steps=steps)
    rows = [masked, generated, y]
    grid = torch.stack(rows, dim=1).reshape(-1, 3, 32, 32)
...
def main():
    ...
    model = Denoiser(base_channels=args.base_channels, t_dim=args.t_dim).to(device)
    ...
    sample_y = sample_y[:10].to(device)
