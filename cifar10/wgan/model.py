...
class Generator(nn.Module):
    def __init__(self, z_dim=100, base_channels=128, slope=0.2):
        super().__init__()
        b = base_channels
        self.in_conv = MuonConv2d(4, b, 3, padding=1)
        ...
        self.z_proj = nn.Linear(z_dim, b * 4 * 8 * 8)
        ...
        self.out_conv = MuonConv2d(b, 3, 3, padding=1)
...
class Critic(nn.Module):
    def __init__(self, base_channels=128, slope=0.2):
        super().__init__()
        b = base_channels
        self.net = nn.Sequential(
            MuonConv2d(7, b, 3, padding=1),
            nn.LeakyReLU(slope),
